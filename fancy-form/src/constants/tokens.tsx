...
export function getLatestTokens(tokens: Token[]): Token[] {
  // Sort tokens by date in descending order
  const sortedTokens = tokens.sort(
    (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
  );

  // Use a Map to keep track of the latest token for each currency
  const latestTokensMap = new Map<string, Token>();

  sortedTokens.forEach((token) => {
    if (!latestTokensMap.has(token.currency)) {
      latestTokensMap.set(token.currency, token);
    }
  });

  // Convert the Map values to an array
  return Array.from(latestTokensMap.values());
}

export const tokens = getLatestTokens(rawTokens);
